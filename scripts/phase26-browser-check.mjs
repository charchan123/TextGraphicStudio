...
try {
  ...
  for(const [i,hex,w] of [[1,'#000000',3],[2,'#FFFFFF',6],[3,'#000000',4]]){await openStroke(i,true);await check(`フチ${i}の使用状態を部分適用`,true);await toggle(`範囲のフチ${i}を使用`,true);await check(`フチ${i}の色を部分適用`,true);await color(`範囲のフチ${i}の色`,hex);await check(`フチ${i}の幅を部分適用`,true);await number(`範囲のフチ${i}の幅`,w);}
  ...
  for(let i=1;i<=3;i++){await check(`フチ${i}の使用状態を部分適用`,false);await check(`フチ${i}の幅を部分適用`,false);if(i!==1)await check(`フチ${i}の色を部分適用`,false);}
  ...
  await check('フチ2の使用状態を部分適用',true);await toggle('範囲のフチ2を使用',false);assert.equal(await page.getByRole('switch',{name:'範囲のフチ3を使用',exact:true}).isDisabled(),true);await apply();
  ...
  await toggle('範囲のフチ2を使用',true);await toggle('範囲のフチ3を使用',true);await apply();const partialOn=await template('partial-contiguous-on');assert.deepEqual(partialOn.partialStyles.at(-1).strokes,{'1':{enabled:true},'2':{enabled:true},'3':{enabled:true}});
  await check('フチ1の使用状態を部分適用',true);await toggle('範囲のフチ1を使用',false);await apply();const partialAllOff=await template('partial-contiguous-all-off');assert.deepEqual(partialAllOff.partialStyles.at(-1).strokes,{'1':{enabled:false},'2':{enabled:false},'3':{enabled:false}});assert.deepEqual(partialAllOff.partialStyles[0],partialContinuous.partialStyles[0]);
